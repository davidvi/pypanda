...
class lioness(panda):
    ...
    def __lioness(self):
        ...
        def lioness_loop(condition, number_conditions, flat_panda_network, expression_matrix, motif_matrix, ppi_matrix):
            ...
            subset_panda_network = subset_panda_network.flatten()
        ...
        flat_panda_network = self.panda_network.transpose().flatten()
        number_conditions = self.expression_data.shape[1]
        self.lioness_network = np.zeros((self.num_genes*self.num_tfs, number_conditions))
        self.lioness_network = map(functools.partial(lioness_loop, number_conditions = number_conditions, flat_panda_network = flat_panda_network, expression_matrix = self.expression_matrix, motif_matrix = self.motif_matrix, ppi_matrix = self.ppi_matrix), range(0, number_conditions))
    ...
    def __lioness_results_data_frame(self):
        ...
        self.export_lioness_results = pd.DataFrame(self.lioness_network.transpose())
